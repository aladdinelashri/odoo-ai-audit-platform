#!/usr/bin/env python3
"""
Odoo AI Audit Platform — System Verification Suite V3
======================================================
Fixed for ACTUAL API contract:
  - query_ast (not ast_query)
  - schedule (not schedule_config)
  - recipients (not email_recipients)
  - status: "active" (not is_active: true)
  - /reports/ with trailing slash

Usage:
    python verify_system_v3.py --admin-user admin --admin-pass Admin123ChangeMe --verbose
"""
...
if access_token:
    ...
    # Create — use ACTUAL field names from your API
    ...
    if ok:
        ...
        log_test("Create report returns query_ast", "PASS" if "query_ast" in report else "FAIL")
    else:
    ...
    if created_report_id:
        ...
        ok = session.put(
            ...
            json={"name": "Updated Verification Report", "status": "inactive"},
            headers=auth_headers,
            timeout=10
        )
        if ok.status_code == 200:
            data = ok.json()
            log_test("Update report", "PASS", f"new_name={data.get('name')}, status={data.get('status')}")
        else:
...
print(f"\n{BOLD}Detailed results written to: verification_report_v3.json{RESET}")

with open("verification_report_v3.json", "w") as f:
    json.dump({
        "summary": {
            "total": total,
            "passed": passed,
